...
=====================================================
Images are grouped by the 9 mirror-position labels (pos0 … pos8) where
pos4 = (0,0) shift.  For every position group the full PSF / MTF pipeline
is run and the results are saved together so positions can be compared.

By default, reads pinhole sweep images from ../calibration_beam_shift/data/
and writes results to ./results/.

Usage:
    python analyze_psf_mtf.py                     # use defaults
    python analyze_psf_mtf.py <folder>            # custom data folder
    python analyze_psf_mtf.py --output-dir DIR    # custom output
"""
...
def main():
    ...
    default_folder = os.path.join(script_dir, '..', 'calibration_beam_shift', 'data')
    ...
    parser.add_argument("folder", nargs='?', default=default_folder,
                        help="Folder containing sweep images (default: ../calibration_beam_shift/data/)")
    ...
    folder = args.folder
    out_dir = args.output_dir if args.output_dir else default_output
