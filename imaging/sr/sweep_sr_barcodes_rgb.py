#!/usr/bin/env python3
"""
sweep_sr_barcodes_rgb.py

Super-resolution processing for all sessions in imaging/rgb_barcodes/.

Data layout
-----------
  imaging/rgb_barcodes/<session>/
      dist+0.0mm_lap*/                   ← best-focus distance folder
          shift*_1.00px_cycle0_rep0/     ← 9 images (3×3 grid), rep 0
          shift*_1.00px_cycle0_rep1/     ← same positions, rep 1
              pos0_(-x,+y).png
              pos2_(+x,+y).png
              pos6_(-x,-y).png
              pos8_(+x,-y).png
...
Nominal 1.0 sensor-pixel diagonal shifts → 0.5 red-LR-pixel shifts:
  pos0 (-x,+y)  →  (dy, dx) = (+0.5, -0.5) LR px
  pos2 (+x,+y)  →  (dy, dx) = (+0.5, +0.5) LR px
  pos6 (-x,-y)  →  (dy, dx) = (-0.5, -0.5) LR px
  pos8 (+x,-y)  →  (dy, dx) = (-0.5, +0.5) LR px
...
Output
------
  imaging/rgb_barcodes/<session>/sr_output/
      native_2x.png
      SAA.png
      SAA_IBP.png
      LR_red_mean.png
...
def load_session(session_dir):
    ...
    for rep_idx, rd in enumerate(rep_dirs):
        lr_frames = [extract_red(load_gray(find_pos_image(rd, pos_prefix)))
