#!/usr/bin/env python3
"""
sweep_sr_barcodes_rgb_cal.py

Like sweep_sr_barcodes_rgb.py but uses CALIBRATED per-corner shifts instead
of nominal ±0.5 red-LR pixels.  Shifts are obtained from:

    imaging/calibration/psf_images/shifts.csv

using superposition of the x-sweep and y-sweep calibration data:

    dx_cal_sensor = dx(x-sweep, pos, |tilt_x_deg|) + dx(y-sweep, pos, |tilt_y_deg|)
    dy_cal_sensor = dy(x-sweep, pos, |tilt_x_deg|) + dy(y-sweep, pos, |tilt_y_deg|)

For RGGB images, 1 sensor pixel = 2 red-LR pixels, so:
    dx_cal_red_lr = dx_cal_sensor / 2
    dy_cal_red_lr = dy_cal_sensor / 2

HR output is 1536 × 2048 (= full sensor resolution).
...
DATA_ROOT     = os.path.join(os.path.dirname(SCRIPT_DIR), 'rgb_barcodes')
...
UPSAMPLE_FACTOR = 2     # red-LR → sensor resolution
f               = UPSAMPLE_FACTOR
...
def get_calibrated_shifts_red_lr(focus_dir, cal):
    """
    Read metadata.json from the first rep dir.
    Return list of (dy_cal, dx_cal) in RED-LR pixel units for the 4 diagonal corners.
    (sensor shifts divided by 2 for Bayer RGGB subsampling)
    """
    ...
    for pos_prefix, pos_label in CORNER_POSITIONS:
        ...
        # sensor-pixel shifts → red-LR-pixel shifts (÷2 for Bayer)
        dx_cal = (dx_x + dx_y) / 2.0
        dy_cal = (dy_x + dy_y) / 2.0

        shifts.append((dy_cal, dx_cal))
        print(f'    {pos_prefix} {pos_label:10s}: '
              f'sensor dx={dx_x+dx_y:+.4f} dy={dy_y+dy_x:+.4f}  '
              f'→ red-LR dx={dx_cal:+.4f} dy={dy_cal:+.4f} px')
...
def extract_red(img_array):
    """Extract red channel from RGGB Bayer raw (even rows, even cols)."""
    return img_array[0::2, 0::2].astype(np.float64)


def load_raw(path):
    """Load a raw PNG and return as float64 array (may be mono or colour-coded)."""
    return np.array(Image.open(path), dtype=np.float64)


def load_red(path):
    """Load image and return the red Bayer channel as float64."""
    img = load_raw(path)
    if img.ndim == 3:
        # Colour-coded: treat first channel as raw; or use mean and subsample
        raw = img[:, :, 0]  # assume R channel in PNG encodes sensor red
    else:
        raw = img
    return extract_red(raw)
...
def load_session(session_dir, cal):
    """
    Load the 4 diagonal-corner red-channel LR frames per rep and compute calibrated shifts.

    Returns:
        all_reps     : list of N_reps lists, each containing 4 red-channel LR arrays
        shifts_cal   : list of 4 (dy_cal, dx_cal) in red-LR pixel units
        session_name : str
    """
    ...
    shifts_cal = get_calibrated_shifts_red_lr(focus_dir, cal)

    all_reps = []
    for rep_idx, rd in enumerate(rep_dirs):
        lr_frames = [load_red(find_pos_image(rd, pos_prefix))
...
def save_comparison(hr_images, mean_lr_red, out_dir, title):
    ...
    ds = 4
    axes[0, 0].imshow(mean_lr_red[::ds, ::ds], cmap='gray', interpolation='nearest')
    axes[0, 0].set_title('Red-LR mean', fontsize=9)
    axes[0, 0].axis('off')
    ...
    lr_zoom = ndi_zoom(mean_lr_red, 2, order=3)
    axes[1, 0].imshow(lr_zoom[cr, cc], cmap='gray', interpolation='nearest')
    axes[1, 0].set_title('Red bicubic 2× (mean)', fontsize=8)
    axes[1, 0].axis('off')
    for i, (name, img) in enumerate(hr_images.items(), 1):
    ...
    plt.suptitle(f'RGB SR cal (red ch) — {title}', fontsize=10)
    plt.tight_layout()
...
def process_session(session_dir, psf_kernel, cal):
    ...
    for rep_idx, lr_frames in enumerate(all_reps):
        ...
        mean_lr_red = np.mean(lr_frames, axis=0)
        print(f'  Rep {rep_idx} — Red-LR frame shape: {mean_lr_red.shape}  '
              f'→ HR will be {mean_lr_red.shape[0]*f} × {mean_lr_red.shape[1]*f}')

        print('    Computing Native-2x ...')
        t0 = time.time()
        native_hr = ndi_zoom(mean_lr_red, f, order=3)
        ...
        arr_lr = np.clip(mean_lr_red, 0, 255).astype(np.uint8)
        Image.fromarray(arr_lr).save(os.path.join(out_dir, 'LR_red_mean.png'))
        print('    Saved LR_red_mean.png')

        save_comparison(hr_images, mean_lr_red, out_dir, f'{session_name} rep{rep_idx}')
