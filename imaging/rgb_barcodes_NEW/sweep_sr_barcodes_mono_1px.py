#!/usr/bin/env python3
"""
sweep_sr_barcodes_mono_1px.py

Super-resolution processing for mono camera images with 1-pixel shift,
downsampled to half resolution to simulate Bayer red-channel extraction.

Data layout
-----------
  MONO_BARCODES_1PIXELSHIFT_*/
      mono_nominal_1.0px_settle50ms/
      mono_special_<timestamp>_1.0px_settle50ms/
...
Images are 1536×2048 uint8 grayscale (mono camera, no Bayer).

Downsampling
------------
To match the Bayer red-channel workflow, we spatially downsample by 2×:
  img[0::2, 0::2]  →  768 × 1024 LR pixels
1 sensor pixel shift ≡ 0.5 downsampled-LR pixel
Upsample factor 2 → 1536 × 2048 HR

Shift geometry (same as RGB/Bayer case)
---------------------------------------
...
PSF: Gaussian (default sigma=1.0, size=7)
...
Output
------
  MONO_BARCODES_1PIXELSHIFT_*/<sr_output_dir>/<calib_dir_name>/rep00/
"""
...
def downsample_2x(img):
    """Spatially downsample by 2× (take every other pixel)."""
    return img[0::2, 0::2].copy()


def find_calib_dirs(session_dir):
    """Return sorted list of mono_* calibration sub-directories."""
    dirs = sorted([
        os.path.join(session_dir, d) for d in os.listdir(session_dir)
        if d.startswith('mono_') and os.path.isdir(os.path.join(session_dir, d))
    ])
    if not dirs:
        raise FileNotFoundError(f'No mono_* sub-dirs in {session_dir}')
    return dirs
...
def load_run(calib_dir, rep_idx):
    """Load the 4 corner downsampled LR frames for one calibration × one rep."""
    ...
    for corner_idx, label, (dy, dx) in CORNER_POSITIONS:
        ...
        lr_frames.append(downsample_2x(load_gray(path)))
        shifts_lr.append((dy, dx))
        print(f'    corner{corner_idx} {label:10s}: '
              f'shift_lr = (dy={dy:+.1f}, dx={dx:+.1f}) LR px')
...
def make_gaussian_psf(halfwidth, sigma):
    ...
    kernel /= kernel.sum()
    print(f'  Gaussian PSF: {size}x{size}, sigma={sigma:.2f}')
    return kernel
...
def save_comparison(hr_images, mean_lr, out_dir, title):
    ...
    plt.suptitle(f'Mono downsampled SR — {title}', fontsize=10)
    plt.tight_layout()
...
def process_run(calib_dir, rep_idx, session_dir, psf_kernel, output_root):
    ...
    single_lr = lr_frames[0]  # single frame (corner0) for Raw baseline
    mean_lr   = np.mean(lr_frames, axis=0)
    print(f'  LR frame shape: {single_lr.shape}')

    print('  Computing Native-2x (single frame) ...')
    t0 = time.time()
    native_hr = ndi_zoom(single_lr, f, order=3)
...
def main():
    parser = argparse.ArgumentParser(description='SR for mono 1px-shift barcodes (downsampled)')
    ...
    # Find MONO_BARCODES_1PIXELSHIFT_* session directories
    sessions = sorted([
    ...
    if not sessions:
        raise FileNotFoundError(f'No MONO_BARCODES_1PIXELSHIFT_* folders in {DATA_ROOT}')
