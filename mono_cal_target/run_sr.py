#!/usr/bin/env python3
"""
sweep_sr.py  --  Mono calibration-target super resolution

Processes mono calibration target images (1536x2048 grayscale).
...
def main():
    parser = argparse.ArgumentParser(
        description='Super-resolution for mono calibration target images')
    ...
    sessions = sorted([
        ...
        if os.path.isdir(os.path.join(data_dir, d))
            and os.path.exists(os.path.join(data_dir, d, 'center.png'))
    ])
    if not sessions:
        raise FileNotFoundError(f'No session folders with center.png found in {data_dir}')
