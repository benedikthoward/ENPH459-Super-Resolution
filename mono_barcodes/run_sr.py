#!/usr/bin/env python3
"""
run_sr.py  --  Mono barcode super resolution

Processes all sessions in data/ for monochrome barcode images.
...
Images are 1536x2048 uint8 mono (no Bayer pattern).

Shift geometry
--------------
The 4 diagonal corners give 2-D coverage with nominal 0.5 sensor-pixel shifts:
  corner0 (-x,+y)  ->  (dy, dx) = (+0.5, -0.5) px
  corner1 (+x,+y)  ->  (dy, dx) = (+0.5, +0.5) px
  corner2 (-x,-y)  ->  (dy, dx) = (-0.5, -0.5) px
  corner3 (+x,-y)  ->  (dy, dx) = (-0.5, +0.5) px
...
SR methods
----------
  Native-2x  : bicubic 2x upsample of the mean of all 4 LR frames
...
def load_session(session_dir):
    ...
    for ri in rep_indices:
        ...
        print(f'    rep {ri:02d}: loaded 4 corner frames')
...
def main():
    parser = argparse.ArgumentParser(
        description='Super-resolution for mono barcode images')
